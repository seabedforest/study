...
with open('./CustomerSurvival.csv', 'r') as f:
    ...
    data = np.array(data, dtype={'names': ['index', 'pack_type', 'extra_time', 'extra_flow',
...
# 分析一下额外流量
print('流失用户剩余流量(平均):', loss_data['extra_flow'].mean())
print('未流失用户剩余流量(平均):', unloss_data['extra_flow'].mean())
...
# 分析关联购买asso_pur 与 集团用户 group_user 对流失率的影响
